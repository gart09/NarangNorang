	...
	public LoginResultDto checkRefreshToken(String refreshToken) {
		try {
			if (jwtUtil.validateRefreshToken(refreshToken) == null) {
				log.info("이건가");
				return LoginResultDto.builder().result("fail").build();
			}
			...
			log.info("Data:" + refreshToken);
			log.info("DB:" + optRefreshToken.get().getTokenKey());
			...
			if (optRefreshToken.isPresent() && optRefreshToken.get().getTokenKey().equals(refreshToken)) {
				...
				log.warn("RefreshToken mismatch or not found for {}", userId);
				return LoginResultDto.builder().result("fail").build();
			}
		} catch (Exception e) {
			log.warn("Get AccessToken failed: {}", e.getMessage());
			return LoginResultDto.builder().result("fail").build();
		}
	}
}
